...
REM Curl v. 8.11.1 was the last that used buildconf.bat on Windows and v. 1.12.0 is the first that does not use this bat file
REM echo Creating configuration
REM call buildconf.bat && cd %CURL_DIR%\winbuild || ( call :last_message "buildconf.bat failed with %ERRORLEVEL%" & exit /b 2 )
cd %CURL_DIR%\winbuild || ( call :last_message "Cannot change directory to winbuild" & exit /b 2 )
echo.
...
start /wait cmd /c call %VCVARSALL% x86 ^&^& nmake /f Makefile.vc mode=dll RTLIBCFG=static GEN_PDB=yes MACHINE=x86 WINBUILD_ACKNOWLEDGE_DEPRECATED=yes || ( call :last_message "Cannot compile on x86" & exit /b 11 )


echo Building x64 config in a different window, please stand by ...
rem if you need to see something from the output just ad an ^&^& pause  after nmake command
start /wait cmd /c call %VCVARSALL% x64 ^&^& nmake /f Makefile.vc mode=dll RTLIBCFG=static GEN_PDB=yes MACHINE=x64 WINBUILD_ACKNOWLEDGE_DEPRECATED=yes || ( call :last_message "Cannot compile on x64" & exit /b 12 )
