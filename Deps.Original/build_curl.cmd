...
git clone %CURL_REPO% %CURL_DIR% && cd %CURL_DIR% && git checkout %CURL_BRANCH% || ( call :last_message "cannot get curl sources" & exit /b 1 )
...
del /s /q %COMPILED_FOLDER%\dll\win32\libcurl.* > nul 2>&1
del /s /q %COMPILED_FOLDER%\dll\x64\libcurl.*   > nul 2>&1
rmdir /s /q %COMPILED_FOLDER%\include\curl      > nul 2>&1
mkdir %COMPILED_FOLDER%\include\curl            || ( call :last_message "cannot create curl inlcude dir" & exit /b 24 )
copy %CURL_DIR%\builds\libcurl-vc-x86-release-dll-ipv6-sspi-winssl\bin\libcurl.* %COMPILED_FOLDER%\dll\win32         || ( call :last_message "cannot copy new curl" & exit /b 31 )
copy %CURL_DIR%\builds\libcurl-vc-x86-release-dll-ipv6-sspi-winssl\lib\libcurl.* %COMPILED_FOLDER%\dll\win32         || ( call :last_message "cannot copy new curl" & exit /b 32 )
copy %CURL_DIR%\builds\libcurl-vc-x64-release-dll-ipv6-sspi-winssl\bin\libcurl.* %COMPILED_FOLDER%\dll\x64           || ( call :last_message "cannot copy new curl" & exit /b 33 )
copy %CURL_DIR%\builds\libcurl-vc-x64-release-dll-ipv6-sspi-winssl\lib\libcurl.* %COMPILED_FOLDER%\dll\x64           || ( call :last_message "cannot copy new curl" & exit /b 34 )
copy %CURL_DIR%\builds\libcurl-vc-x86-release-dll-ipv6-sspi-winssl\include\curl\*.* %COMPILED_FOLDER%\include\curl   || ( call :last_message "cannot copy new curl includes" & exit /b 35 )
...
:cleanup
	rem Wait a few seconds seconds, TortoiseGit or others my be accessing the folder.
	ping 127.0.0.1 -n 4 >nul
	rem Proceed to cleanup
	cd %~dp0
	set CLEAN_CMD=rmdir /s /q %CURL_DIR%
	%CLEAN_CMD% > nul 2>&1 || (echo Cannot cleanup sources folder, rerun by yourself: %CLEAN_CMD%) 
	echo %MARK%
	popd
	GOTO:eof
